...
$(function(){
	...
	$('input[name=im_content]').val('${thumbInfo.im_content }');
	
    $('#im_writer').val("${LOGIN_MEMBERINFO.mem_id}");
    $('#im_mail').val("${LOGIN_MEMBERINFO.mem_mail}");

	$('form[name=im_form]').submit(function(){
		$(this).attr("action", "${pageContext.request.contextPath }/user/thumb/thumbUpdate.do?im_no=${thumbInfo.im_no }");
		$(this).append('<input type="hidden" name="im_no" value="${boardInfo.im_no}" />');
		$(this).append('<input type="hidden" name="im_writer" value="${thumbInfo.im_writer}" />');
