...
<script type="text/javascript">
   $(function(){
      ...
      $('.thumbTable tr').click(function(){
         if(eval('${empty LOGIN_ADMININFO.admin_id}')){
            alert("로그인 후 게시글 확인이 가능합니다.");
         }else{
            var im_no = $(this).find('input[name=th_bo_no]').val();         
            location.href = "${pageContext.request.contextPath }/admin/thumb/thumbView.do?im_no="+im_no;
         }
      });
      
      $('input[value=삭제]').click(function() {
			location.href = '${pageContext.request.contextPath }/admin/thumb/deleteThumb.do?im_no=';
	  });
      ...
      $('.btn2').click(function(){
			location.href = '${pageContext.request.contextPath }/admin/thumb/thumbForm.do';
      });
      ...
      <tr valign="top">
         ...
         <td width="790"> 
            ...
            <div class="title">썸네일</div>
	            	<table border="1" cellspacing="1" cellpadding="1" class="thumbTable">
		                  <c:forEach items="${thumbList }" var="thumbInfo">
                 			 <tr class="tbl-item" >
	                           <td class="img" width="50px" height="50px">
	                              <c:forEach items="${thumbInfo.items}" var="item">
	                              <img src="/files/${item.file_save_name }"  width="150px" height="100px"/>
	                              </c:forEach>
	                           </td>
